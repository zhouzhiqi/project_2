...
##################################################
    ...
    def GetOneDay(self, day_clicks,  hour, days, user, C17):
        ...
        return: onehot后的数据
        """
        click_tmp = np.zeros((12),dtype=np.int8)
        # 传入列名
        ...
        #  'click_rate<0.05', '0.4<click_rate', 'is_C17_Fir', 'is_C17_Sec']
        ...
        elif 1 < click_times <= 5: click_tmp[1]=1
        elif 5 < click_times <= 10: click_tmp[2]=1
        else: click_tmp[3]=1
        # 处理浏览次数
        total_times = click_hour[1]
        if     total_times <= 5: click_tmp[4]=1
        elif 5 < total_times <= 20: click_tmp[5]=1
        elif 20 < total_times <= 100: click_tmp[6]=1
        else: click_tmp[7]=1
        click_rate = click_times/total_times
        if click_rate < 0.05: click_tmp[8]=1
        elif 0.4 < click_rate: click_tmp[9]=1
        # 处理感兴趣广告类型
