...
def LRPredict(X_train, y_train, model_path, c):
    #保存模型
    lr_model = joblib.load(model_path+'LR_C{0}.sklearn'.format(c))
    #进行评价
...
y_train = ss.load_npz(output_path+'{0}_y_more{1}.npz'.format(file_name, threshold), )
...
#train_preds_llr = LRPredict(X_train, y_train, model_path, c)
#取平均值
...
submission.insert(loc=1, column='click', value=train_preds_xgb)
